    ...
    <script>
        ...
        layui.use(['table', 'form'], function () {
            ...
            tableIns = table.render({
                id: 'table'
                , elem: '#table'
                , url: '/finance/index.php/Admin/Discount/getList' //数据接口
                , page: true //开启分页
                ...
                , cols: [[ //表头
                    ...
                    , { field: 'discount_id', title: 'ID', width: 150 }
            ...
            table.on('tool(table_filter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if (layEvent === 'del') { //删除
                    ...
                    layer.confirm('真的删除此条数据吗？', function (index) {
                        ...
                        $.post('/finance/index.php/Admin/Discount/del', {
                            "discount_id": obj.data.discount_id,
                        }, function (res) {
                            res = JSON.parse(res);
                            if (res.res == 0) {
                                layer.msg('删除成功~', {
        ...
        */
        ...
        $('#add').on('click', function () {
            layer.open({
                ...
                area: ['893px', '600px'],
                content: '/finance/index.php/Admin/Discount/add'
        ...
        $(document).on('click', '#removeAll', function () {
            ...
            layer.confirm('确定删除这些商品？', function (index) {
                ...
                for (var i = 0; i < o.data.length; i++) {
                    id += "'" + o.data[i].discount_id + "',";
                }
                id = id.substring(0, id.length - 1);

                $.post('/finance/index.php/Admin/Discount/removes', {
                    'discount_id': id
                }, function (res) {
        ...
        function saveInfo(post, f) {
            $.post('<?php echo U("discount/saveInfo");?>', post, function (res) {
                console.log(res);
                res = JSON.parse(res);
                if (f != null) {
                    f(res);
